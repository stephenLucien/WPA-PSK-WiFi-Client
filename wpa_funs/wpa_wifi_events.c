...
static void* _event_monitor_thread(void* userdata) {
	int ret = 0;
	char reply[BUF_LEN] = {0};
	size_t reply_len = BUF_LEN;
	while (g_thread_running && mctrl) {
		if (1 == wpa_ctrl_pending(mctrl)) {
			reply_len = BUF_LEN - 1;
...
static void* _monitoring_results_thread(void* userdata) {
	int ret = 0;
	enum WifiEventType event_type;
	while (g_thread_running && mctrl) {
		sleep(1);
...
int wpa_wifi_event_start() {
	...
	g_thread_running = 1;
	ret = pthread_create(&mpid, NULL, &_event_monitor_thread, NULL);
	if (ret) {
		printf("[%s, %d] thread is not created.\n", __FUNCTION__, __LINE__);
	}
	ret = pthread_create(&mpid, NULL, &_monitoring_results_thread, NULL);
	if (ret) {
		printf("[%s, %d] thread is not created.\n", __FUNCTION__, __LINE__);
	}
	return ret;
